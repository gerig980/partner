...
export default class EventController {
  ...
  buildUIModel() {
    ...
    if (this.store.getAreTooltipsFullscreen()) {
      ...
      this.UIRects[consts.RECT_OPENED_FULLSCREEN_TOOLTIP] = () => { return this.store.getOpenedFullscreenTooltipRect() }
    }

    if (this.store.state.fullscreen.enable_fullscreen_mode) {
      this.UIRects[consts.RECT_BUTTON_FULLSCREEN] = () => { return this.store.getFullscreenButtonRect() }
    }

    if (this.store.state.zooming.enable_zooming) {
      if (this.store.state.zooming.enable_zoom_buttons) {
        this.UIRects[consts.RECT_BUTTON_ZOOM_IN] = () => { return this.store.getZoomInButtonRect() }
        this.UIRects[consts.RECT_BUTTON_ZOOM_OUT] = () => { return this.store.getZoomOutButtonRect() }
      }

      if (this.store.state.zooming.enable_navigator) {
        this.UIRects[consts.RECT_NAVIGATOR] = () => { return this.store.getNavigatorRect() }
      }
    }

    if (this.store.state.artboards.length > 1) {
      this.UIRects[consts.RECT_ARTBOARD_SELECT_MENU] = () => { return this.store.getArtboardSelectRect() }
    }

    if (this.store.state.object_list.enable_object_list) {
      this.UIRects[consts.RECT_BUTTON_OPEN_MENU] = () => { return this.store.getOpenMenuButtonRect() }
      this.UIRects[consts.RECT_BUTTON_CLOSE_MENU] = () => { return this.store.getCloseMenuButtonRect() }
      this.UIRects[consts.RECT_MENU] = () => { return this.store.getMenuRect() }
    }
  }
  createEvents() {
  ...
  handleEventKeyUp(e) {
    ...
    if (document.querySelector(`[data-imp-id="${this.store.getID()}"] input:focus`)) {
      this.store.dispatch('updateSearch', { searchString: document.querySelector(`[data-imp-id="${this.store.getID()}"] input:focus`).value })
    }
  }
  handleEventResize() {
  ...
  handleEventChange(e) {
    if (e.target.classList.contains('imp-ui-layers-select') && e.target.closest(`[data-image-map-id="${this.store.getID()}"]`)) {
      this.store.dispatch('changeArtboard', { artboardId: e.target.value, zoomOut: true })
    }

    if (e.target.closest(`[data-imp-id="${this.store.getID()}"] .imp-search-box-input-wrap`)) {
  ...
  doClickUI(e) {
    if (this.UIClickedRect) {
      // console.log('Clicked rect: ' + this.UIClickedRect)
      ...
      if (this.UIClickedRect == consts.RECT_OPENED_FULLSCREEN_TOOLTIP) {
        if ((e.target.closest('.imp-tooltip-content') === null && !e.target.classList.contains('imp-tooltip-content')) || e.target.closest('.imp-tooltip-close-button') || e.target.classList.contains('imp-tooltip-close-button')) {
  ...
  async doClickObject() {
    if (this.objectIdClicked !== false && this.getDistanceBetweenCoordinates(this.eventCoordinates, this.eventCoordinatesStart) < 10) {
      ...
      this.store.dispatch('highlightObject', { objectId: this.objectIdUnderEvent, showTooltip: true, hideAllTooltips: false })
      return true
    }
    return false
  }
  async doMouseOverObject(e) {
    ...
    if (this.objectIdUnderEvent !== false) {
      ...
      this.store.dispatch('highlightObject', { objectId: this.objectIdUnderEvent, showTooltip: showTooltip, hideAllTooltips: false })
  ...
  async doMenuEnd(e) {
    ...
    if (this.menuListItemIDUnderEvent) {
      await this.store.dispatch('changeArtboard', { artboardId: this.store.getArtboardIdForObject({ id: this.menuListItemIDUnderEvent }), zoomOut: false })

      if (!this.store.getObject({ id: this.menuListItemIDUnderEvent }).static) {
        if (this.store.getImageMap().config.zooming.zoom_on_object_click) {
          await this.store.dispatch('focusObject', { objectId: this.menuListItemIDUnderEvent })
        }
        await this.store.dispatch('highlightObject', { objectId: this.menuListItemIDUnderEvent, showTooltip: true, hideAllTooltips: false })
  ...
  isPointInsideRect(point, rect) {
    ...
    return point.x > rect.offset.left
  ...
  getMenuListItemIdUnderEvent(e) {
    ...
    const listItemId = e.target.dataset.listItemId || e.target.closest('[data-list-item-id]')
